...
define( 'LP_ADDON_CERTIFICATES_VER', '2.2.6' );
...
class LP_Addon_Certificates {
		...
		if ( $this->finish_course_data ) {
			if ( $cert_id = get_post_meta( $this->finish_course_data['course_id'], '_lp_cert', true ) ) {
				...
				if ( $email->get_content_format() == 'text/html' ) {
					$email_cert_link    = sprintf( '<a href="%s">%s</a>', $email_cert_raw_link, __( 'View', 'learnpress' ) );
	...
	public function add_tab_settings( $tabs ) {
		...
		$tabs['certificates'] = __( 'Certificates', 'learnpress' );
		return $tabs;
	}
	...
	public function admin_head() {
		...
		<script type="text/javascript">
			var certificate_localize = {
				import               : '<?php _e( 'Import from Zip', 'learnpress' );?>',
				import_sample        : '<?php _e( 'Import Sample', 'learnpress' );?>',
				upload_title         : '<?php _e( 'Select a zip file', 'learnpress' );?>',
				confirm_import_sample: '<?php _e( 'Do you want to import sample of certificate?', 'learnpress' );?>'
			}
		</script>
		<?php
	}
	...
	public function import_from_file( $zip_file ) {
		...
		return
			array(
				...
				'success'  => 1,
				'message'  => __( 'Import certificate successfully', 'learnpress' )
			);
	}
	...
	public function row_actions( $actions, $post ) {
		...
		if ( $post->post_type == "lp_cert" ) {
			$actions['export_cert'] = sprintf( '<a href="%s">%s</a>', admin_url( 'edit.php?post_type=lp_cert&export=' . $post->ID ), __( 'Export', 'learnpress' ) );
		}
		return $actions;
	}
	...
	function load_field() {
		...
		if ( empty( $field_options['fieldType'] ) ) {
			throw new Exception( __( 'Invalid field type!', 'learnpress' ) );
		}
	...
	function register_post_type() {
		...
		register_post_type( 'lp_cert',
			array(
				'labels'             => array(
					'name'          => __( 'Certificate', 'learnpress' ),
					'menu_name'     => __( 'Certificates', 'learnpress' ),
					'singular_name' => __( 'Certificate', 'learnpress' ),
					'add_new_item'  => __( 'Add New Certificate', 'learnpress' ),
					'edit_item'     => __( 'Edit Certificate', 'learnpress' ),
					'all_items'     => __( 'Certificates', 'learnpress' ),
				),
				'public'             => false,
	...
	function add_meta_boxes() {
		add_meta_box(
			...
			__( 'Certificate Design', 'learnpress' ),
	...
	function columns_head( $columns ) {
		if ( get_post_type() == 'lp_cert' ) {
			$columns['courses'] = __( 'Courses', 'learnpress' );
		}
		$columns['certificate'] = __( 'Preview', 'learnpress' );
		return $columns;
	}
	...
	public static function admin_notice() {
		?>
		<div class="error">
			<p><?php printf( __( '<strong>Certificates</strong> addon version %s requires <strong>LearnPress</strong> version %s or higher', 'learnpress' ), LP_ADDON_CERTIFICATES_VER, LP_ADDON_CERTIFICATES_REQUIRE_VER ); ?></p>
		</div>
		<?php
	}
