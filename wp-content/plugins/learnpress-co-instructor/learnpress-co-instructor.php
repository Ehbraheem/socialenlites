...
if ( ! defined( 'LP_ADDON_CO_INSTRUCTOR_PATH' ) ) {
	...
	define( 'LP_ADDON_CO_INSTRUCTOR_VER', '2.0.3' );
...
// get all quizzes from course
	...
	public function learn_press_before_subject_enrolled_course_admin__settings_fields( $settings, $settings_class ) {
        ...
        <tr>
            <th scope="row">
                <label for="learn-press-emails-enrolled-course-send-instructors"><?php _e( 'Send Co-Instructors', 'learnpress' ); ?></label>
            </th>
            <td>
                <input type="hidden"
                       name="<?php echo $settings_class->get_field_name( 'emails_enrolled_course_admin[send_instructors]' ); ?>"
                       value="no"/>
                <input id="learn-press-emails-enrolled-course-send-instructors" type="checkbox"
                       name="<?php echo $settings_class->get_field_name( 'emails_enrolled_course_admin[send_instructors]' ); ?>"
                       value="yes" <?php checked( $settings->get( 'emails_enrolled_course_admin.send_instructors' ) == 'yes' ); ?> />
            </td>
        </tr>
		<?php
	}
